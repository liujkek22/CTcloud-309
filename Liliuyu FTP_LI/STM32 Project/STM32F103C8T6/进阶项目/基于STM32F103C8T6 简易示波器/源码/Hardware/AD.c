...
void AD_Init(void)
{
	...
	GPIO_Iitstructure.GPIO_Pin = GPIO_Pin_1|GPIO_Pin_2 | GPIO_Pin_3 ;//启用0引脚
	GPIO_Iitstructure.GPIO_Speed = GPIO_Speed_50MHz;//引脚速度50MHz
	GPIO_Init(GPIOA,&GPIO_Iitstructure);
	ADC_RegularChannelConfig(ADC1, ADC_Channel_1, 2, ADC_SampleTime_55Cycles5);//设定规则组：使用ADC1，连接3通道，第三序列位置，55.5个ADCCLK的采样周期
